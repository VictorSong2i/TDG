...
describe('generate test data', () => {
  ...
  it('Check contents of csv and json file match',()=>{
    cy.task('unzipping', { path, file}) //Unzip file into its csv and 
    ...
    cy.readFile(csvFilePath).then((csvFile)=>{
      ...
      let fields = lines[0].split(',') //Creates data field array

      expect(lines.length).to.equal(numData+1) //Checks number of entries, +1 because of extra line for field names
      expect(fields.length).to.equal(mergedDetails.length) //Check number of fields
      
      //Compare field name in downloaded file to 
      cy.fixture("dataName.json").then((properties)=>{
          cy.log(properties) //Properties - dictionary for matching field names on website to csv/json field names
      
          //Checks if fields in downloaded file exist in dictionary dataName using expect
          for(let i=0;i < mergedDetails.length; i++){
          // cy.log(Object.keys(properties).find(key => properties[key] == fields[i].substring(1,fields[i].length-1)))
          expect(Object.keys(properties).find(key => properties[key] == fields[i].substring(1,fields[i].length-1))).to.exist 
          }
      })
    })
  })
